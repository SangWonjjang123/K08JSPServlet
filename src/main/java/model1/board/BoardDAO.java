...
public class BoardDAO extends JDBConnect {
	...
	/*
	 * board테이블에 저장된 게시물의 갯수를 카운트하기 위한 메서드 타운트 한 결과값을 통해 목록 출력시 게시물의 순번을 출력한다. 만약
	...
	public List<BoardDTO> selectList(Map<String, Object> map) {
		/*
		 * select한 게시물의 목록은 다수의 레코드가 포함되므로 이를 저장하기 위해 순서를 보장하는 List 계열의 컬렉션이 필요하다
		 * Set컬렉션은 순서를 보장하지 못하므로 게시판 목록을 출력하는 용도로 사용할 수 없다
		 */
		...
		try {
			...
			while (rs.next()) {
				//하나의 레코드를 읽어서추출한 후 DTO객체에 저장한다
				//DTO객체를 생성한 후..
				BoardDTO dto = new BoardDTO();
				//각 멤벼변수에 해당하는 컬럼을 매칭하여 데이터를 저장한다.
				...
				//하나의 레코드를 DTO에 저장한 후 List 컬렉션에 추가한다
				bbs.add(dto);
			}
		} catch (Exception e) {
			System.out.println("게시물 select 중 예외 발생");
			e.printStackTrace();
		}
		return bbs;
